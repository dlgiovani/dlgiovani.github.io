...
export interface GlassIconsMagnetItem {
...
export interface GlassIconsMagnetProps {
    items: GlassIconsMagnetItem[];
    className?: string;
}
...
const GlassIconsMagnet: React.FC<GlassIconsMagnetProps> = ({ items, className }) => {
    ...
    return (
        <div className={`grid gap-[5em] grid-cols-2 md:grid-cols-3 mx-auto py-[3em] overflow-visible ${className || ""
            ...
            {items.map((item, index) => (
                <Magnet padding={50} disabled={false} magnetStrength={10} key={index}>
...
export default GlassIconsMagnet;
